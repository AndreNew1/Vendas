...
namespace Core
{
    public class ProdutoCore: AbstractValidator<Produto>
    {
        ...
        {
            var resposta =Mapper.Map<List<Produto>,List<ProdutoViewRetorno>>(Db.Produtos.OrderBy(x => x.Nome).ToList());

            return resposta.Count == 0 ?
            new Retorno { Status = false, Resultado = "Não existe dados" } :
            new Retorno { Status = false, Resultado = resposta };
        }
        ...
        public Retorno PorPagina(int NumPagina, string Ordenacao, int TamanhoPagina)
        {
            ...
            try
            {
                ...
                if (Ordenacao.ToLower() == "Preco" && NumPagina >= 1)
                    return new Retorno { Status = true, Resultado = (Mapper.Map<List<Produto>,List<ProdutoViewRetorno>>(Db.Produtos.OrderBy(x => x.Valor).Skip((NumPagina - 1) * TamanhoPagina).Take(TamanhoPagina).ToList()), $"Total de paginas: {Paginas}") };

                return Ordenacao.ToLower() == "Nome" && NumPagina >= 1 ?
                      new Retorno { Status = true, Resultado = (Mapper.Map<List<Produto>,List<ProdutoViewRetorno>>(Db.Produtos.OrderBy(x => x.Nome).Skip((NumPagina - 1) * TamanhoPagina).Take(TamanhoPagina).ToList()), $"Total de paginas: {Paginas}") } :
                      new Retorno { Status = true, Resultado = (Mapper.Map<List<Produto>,List<ProdutoViewRetorno>>(Db.Produtos.Take(TamanhoPagina).ToList()), $"Total de paginas: {Paginas}") };
            }
            catch (Exception) { }

            //se paginação é não é possivel
            return new Retorno { Status = true, Resultado = (Mapper.Map<List<Produto>,List<ProdutoViewRetorno>>(Db.Produtos.OrderBy(x => x.DataCadastro).Take(15).ToList()), $"Total de paginas: {Paginas}") };
        }
        ...
        public Retorno ID(string id)
        {
            ...
            return produto == null
                ...
                : new Retorno { Status = true, Resultado = Mapper.Map<ProdutoViewRetorno>(produto) };
        }
